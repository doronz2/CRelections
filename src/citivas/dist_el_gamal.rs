...
impl DistElGamal {
    ...
    pub fn create_valid_shares_list(
        &self,
        comm_list: Vec<CommitmentKeyGen>,
        proof_list: Vec<KeyProof>,
    ) -> Vec<ElGamalPublicKey> {
    ...
    pub fn construct_public_key_from_valid_shares(
        &self,
        pk_list: &Vec<ElGamalPublicKey>,
    ) -> ElGamalPublicKey {
        let global_pk = pk_list
            .iter()
            .fold(BigInt::one(), |prod, key_share| prod * &key_share.h)
    ...
    pub fn construct_shared_public_key(
        &self,
        comm_list: Vec<CommitmentKeyGen>,
        proof_list: Vec<KeyProof>,
    ) -> ElGamalPublicKey {
        let valid_shares_list = &self.create_valid_shares_list(comm_list, proof_list);
        self.construct_public_key_from_valid_shares(valid_shares_list)
    }

    //------ Distributed El Gamal Decryption --------
    pub fn publish_shares_and_proofs_for_decryption(
        &self,
        cipher: &ElGamalCiphertext,
    ) -> DistDecryptEGMsg {
        let a_i = BigInt::mod_pow(
            &cipher.c1,
            &self.share_key_pair.sk.x,
            &self.share_key_pair.pk.pp.p,
        );
    ...
    pub fn verify_proof_for_decryption(
        &self,
        cipher: &ElGamalCiphertext,
        share_and_proof: &DistDecryptEGMsg,
        party_index: i32,
    ) -> bool {
        let statement = DDHStatement {
            pp: self.share_key_pair.pk.pp.clone(),
        ...
        if is_valid.is_err() {
            panic!(
                "share/proof of share {} for decryption is invalid",
                party_index
            );
        } else {
            true
...
mod test {
    ...
    pub fn test_generate_key_from_shares() {
        ...
        let shared_private_key = ElGamalPrivateKey {
            x: (party_1.share_key_pair.sk.x
                + party_2.share_key_pair.sk.x
                + party_3.share_key_pair.sk.x)
                .mod_floor(&pp.q),
            pp: pp.clone(),
        };
    ...
    pub fn test_distributed_EG_decryption() {
        ...
        let valid_shares_for_decryption: Vec<BigInt> = parties
            ...
            .filter(|(party, share_and_proof)| {
                party.verify_proof_for_decryption(
                    &encrypted_msg,
                    share_and_proof,
                    party.party_index,
                )
