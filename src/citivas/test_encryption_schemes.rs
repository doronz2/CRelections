...
pub mod test_encryption {
    use crate::citivas::encryption_schemes::{
        encoding_quadratic_residue, reencrypt, ElGamalCipherTextAndPK, NonMellableElgamal,
    };
    use crate::BigInt;
    use crate::{ElGamal, ElGamalKeyPair, ElGamalPP, ExponentElGamal, SupportedGroups};
    ...
    fn test_basic_el_gamal() {
        ...
        let factor1 = elgamal::ElGamal::encrypt(&BigInt::from(5), &alice_key_pair.pk).unwrap();
        let factor2 = elgamal::ElGamal::encrypt(&BigInt::from(3), &alice_key_pair.pk).unwrap();
    ...
    fn test_exp_el_gamal() {
        ...
        for i in 0..1000000 {
            let res = BigInt::mod_pow(
                &alice_key_pair.pk.pp.g,
                &BigInt::from(i),
                &alice_key_pair.pk.pp.p,
            );
            if res.eq(&dec) {
                plain = BigInt::from(i);
                break;
    ...
    fn test_encrypt_credentials() {
        ...
        let encoded_msg = encoding_quadratic_residue(BigInt::from(17), &pp);
        let encrypted_credential = NonMellableElgamal::encrypt_credential(
            &encoded_msg,
            &key_pair.pk,
            &credential_nonce,
            rid.clone(),
        ...
        let verify_credential =
            NonMellableElgamal::verify_credential(&encrypted_credential, rid, vid, &pp);
    ...
    fn test_reencryption() {
        ...
        .unwrap();
        let ctx_and_pk = ElGamalCipherTextAndPK {
            ctx,
            pk: &key_pair.pk,
