...
impl Voter {
    ...
    pub fn get_key_pair(&self)-> &ElGamalKeyPair{
    ...
    pub fn DVRP_prover(&self, dvrp_input: &DVRP_Public_Input, eta: BigInt) -> DVRP_Proof {
        ...
        let s = BigInt::mod_mul(&BigInt::mod_pow(&self.pp.g, &w, &self.pp.p),
                                &BigInt::mod_pow(&self.get_pk(), &r, &self.pp.p),
                                &self.pp.p);
    ...
    pub fn DVRP_verifier(&self, dvrp_input: &DVRP_Public_Input, dvrp_proof: &DVRP_Proof) -> bool {
        ...
        let nom = BigInt::mod_pow(&self.pp.g, &dvrp_proof.u, &self.pp.p);
        let denom = div_and_pow(&x, &x_tag, &(&dvrp_proof.c + &dvrp_proof.w), &self.pp.p);
        ...
        let s_tag = BigInt::mod_mul(&BigInt::mod_pow(&self.pp.g, &dvrp_proof.w, &self.pp.p),
                                    &BigInt::mod_pow(&self.get_pk(), &dvrp_proof.r, &self.pp.p),
                                    &self.pp.p);
    ...
    pub fn fakeDVRP_prover(&self, dvrp_input: &DVRP_Public_Input) -> DVRP_Proof {
        ...
        let a_tilde: BigInt = (BigInt::mod_pow(&self.pp.g, &u_tilde, &self.pp.p) *
            BigInt::mod_inv(&div_and_pow(&x, &x_tag, &alpha, &self.pp.p)
                            , &self.pp.p)).mod_floor(&self.pp.p);
        let nom = BigInt::mod_pow(&self.pp.g, &u_tilde, &self.pp.p);
        let denom = div_and_pow(&x, &x_tag, &alpha, &self.pp.p);
        ...
        let w_tilde = (&alpha - &c_tilde).mod_floor( &self.pp.q);
        let r_tilde = (&(&(&beta - &w_tilde) * BigInt::mod_inv(&key_pair.sk.x, &self.pp.q)))
            .mod_floor( &self.pp.q);
        DVRP_Proof{
            c: c_tilde,
            w: w_tilde,
